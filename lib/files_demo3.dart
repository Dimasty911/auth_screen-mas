...
class FilesDemoScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: FlutterDemo(storage: CounterStorage(), title: 'Чтение и запись',),
    );
  }
}
...
class _FlutterDemoState extends State<FlutterDemo> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Рябов ДЕ'),
      ),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Text('Кнопка Плюс $_counter ра${_counter == 1 ? '' : 'з'}.'),
            const SizedBox(height: 30,),
            Text('Кнопка Стар $_counter1 ра${_counter1 == 1 ? '' : 'з'}.'),
          ],
        ),
      ),
